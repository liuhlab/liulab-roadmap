...
把这一页放在整个学习路线的第一站，是有意安排的。倒不是因为这些工具时髦，而是它们实实在在改变了后面每一样学习的代价。过去遇到一个问题，可能得跟别人的代码耗上一个星期，如今很多时候就是一场对话的事。真正卡脖子的地方，于是从"能不能把代码写出来"挪到了"有没有把问题想明白"。接下来我们会先认识底下的大语言模型，再聊聊这类工具为什么值得花功夫学，几个名字长得很像的东西分别是什么，从哪里上手，以及怎么用它放大自己的能力。

## 先认识大语言模型

coding agent 的发动机是大语言模型（large language model，LLM），值得先花一点篇幅认识它。它学会的本事说起来只有一样：读了海量的文字之后，给定前面的话，猜下一个词最可能是什么。聊天窗口里那些对答如流，就是这个接龙游戏训练了亿万次之后的样子。记住这一点，后面的很多怪事就好解释了：它不是数据库，说出来的话不保证是真的；它也没有长久的记忆，每次能看到的只有你发给它的那一窗内容，也就是后面会反复提到的上下文窗口。agent 偶尔一本正经地胡说，或者聊久了像换了个人，根子都在这两条上。

用它的人，这几年换了三茬打法。最早是聊天机器人时代，一个问题换一个回答，值得钻研的是怎么把提示词写好，prompt engineering 这个词就是那个年代的产物。后来模型学会了先想再答，推理模型把"一步一步来"从提示词技巧变成了内建的习惯，字斟句酌这件事本身就不那么要紧了。最近的一次变化是把模型放进循环里，给它工具，让它自己读文件、跑命令、看结果、再决定下一步，这就是 agent。人的功夫也跟着挪了地方：从琢磨一句话的措辞，挪到搭一个好用的环境，给清楚的目标、趁手的工具，再加一个能自我检查的抓手。这一页后面讲的"交办而不是口述"，就是这个新打法的日常版本。

想把这些底子一次补齐，可以花半小时过一过 [LLM Fundamentals](https://www.aihero.dev/llm-fundamentals)（英文）。它在浏览器里交互着学，不用装任何东西，讲的正是上面这几件事的更底层：system prompt 到底起什么作用，模型为什么是用数字"思考"的，上下文塞太满为什么反而坏事，工具给太多为什么会变慢。概念不多，但每一个都是以后天天要打的交道。
...
对咱们实验室来说，这些工具已经是必须掌握的东西。2026 年年中发布的 [Claude Science](https://claude.com/product/claude-science) 自带基因组学、单细胞、结构生物学的工具，还能通过 SSH 操作 HPC 集群，用 `sbatch` 提交作业，而不是图省事在登录节点上直接跑。[Edison Scientific](https://www.edisonscientific.com/) 的 Kosmos 则是另一个方向，定位就是"AI 科学家"：自己读文献、分析数据、提出假设，面向生物医药的研发。用不用这些产品另说，可以预见，这类工具以后会越来越普及。

## 三样长得很像的东西

刚开始最值得理清的一件事："聊天机器人""coding agent"，再加上 OpenClaw、Hermes 那一类，看起来是三个同类型的产品，其实不是。它们是同一个模型穿了三件厚薄不同的外衣。模型是同一个，而且不管套哪件外衣，它都跑在数据中心里，不在你的电脑上。真正变的是外面那层 harness（外壳），也就是它能调用哪些工具、被允许碰到你手上的哪些东西。
...
| --- | --- | --- | --- |
| 举例 | claude.ai、ChatGPT | Claude Code、Codex CLI、Kimi Code、Z Code | OpenClaw、Hermes |
...
这个分类法值得记住，因为表里这几条属性，既决定了本事大小，也决定了闯祸规模。聊天机器人删不掉你的文件，因为它根本看不见；coding agent 看得见也动得了，所以才一定要在 git 里用它，这样它的每一处改动都是一段 diff，你能逐行读，也能一键退回。[OpenClaw](https://github.com/openclaw/openclaw)、[Hermes](https://github.com/NousResearch/hermes-agent) 这类 agent 助手走得更远，把模型接进邮件、聊天和日历，不用等你发话就能自己动手。方向确实是朝那边走的，不过这类工具眼下还不成熟，可以慢慢了解，用的时候多留个心眼。
...
Claude Code 也不是唯一的选择，同类工具还有 [Kimi Code](https://www.kimi.com/code/docs/)、智谱的 [Z Code](https://zcode.z.ai/) 等，思路和用法大同小异。Kimi Code 在终端里一行命令就能装好（`curl -fsSL https://code.kimi.com/kimi-code/install.sh | bash`），装完输入 `kimi` 就能开始。这一页讲的概念和习惯在哪家工具上都通用，先挑一个顺手的用熟，比纠结选哪个更重要。
...
实验室自己也维护了[一套 Skills](https://liuhlab.github.io/liulab-compute-skills/)，打包成一个 Claude Code 插件，见[常规实验室支持](../5-lab-code/5.1-routine-support.md)。等你上手以后，工具就是从那儿开始，从"通用"变成"懂我们的数据"。
...
课里用的是 Claude Code，但学完不用怕被绑在这一家工具上：上下文管理、子 agent、hooks、MCP 这些概念在各家 coding agent 上是通用的，换成 Kimi Code、Z Code 照样用得上。
...
- 各家官方文档是日常最常回去查的地方：[Claude Code 文档](https://code.claude.com/docs/zh-CN/overview)、[Kimi Code 文档](https://www.kimi.com/code/docs/)、[Z Code 文档](https://zcode.z.ai/cn)。
