...
命令行、Git 和 GitHub 是实验室日常工作流的基础：敲命令驱动服务器上的计算，Git 记录每一次重要文件的改动，GitHub 把这份历史分享出去，变成协作。磨刀不误砍柴工，这几样加上 Python，就是我们做一切计算和分析的刀；Python 内容多，放在下一页 [Python 基础](1.3-python.md) 里单独讲。这三样恰好也是 coding agent 最依赖的技能，所以学的重点不再是背命令，而是读懂 agent 干了什么，判断它做的事情是否必要。这一页先讲操作系统和计算环境的背景，再挑出值得学的基础，配几份入门材料，再给几个能检查自己是不是真会了的地方。

## 操作系统：Unix 一大家子，Windows 另一支

操作系统（operating system，OS）是开机后第一个启动的程序，硬件、文件和其他所有程序都归它管，你看到的桌面只是它最外面的一层皮。今天常见的桌面系统看着是三个，其实是两个家族。Unix 1969 年诞生在贝尔实验室，后代占了两个：一个是 Linux，1991 年 Linus Torvalds 重写的免费开源内核，如今几乎所有服务器、超算和安卓手机跑的都是它；另一个是 macOS，它的内核 Darwin 是 BSD Unix 的直系后代，正经拿过 UNIX 商标认证。Windows 来自另一支血脉，从 MS-DOS 一路到 Windows NT，路径写法（`C:\` 对 `/`）、命令词汇、设计哲学都和 Unix 家族不一样。

这层家族关系直接决定你的日常体验。Mac 的终端打开就是 Unix：命令、路径、工具链和 Linux 服务器几乎一模一样，在笔记本上练熟的东西，原样搬到服务器上就能用。Windows 自带的 cmd 和 PowerShell 说的是另一套语言，生信教程里的命令粘进去大多不认。好在微软官方的 WSL（Windows Subsystem for Linux）能在 Windows 里装进一个真的 Ubuntu：用管理员权限打开 PowerShell，跑一条 `wsl --install` 就行。而且说到底，实验室的重计算迟早要 SSH 到 Linux 服务器上做，WSL 是先把家门口这一步也换成同一种语言。

所以命令行相关的工作，我们都推荐在 Linux 或 macOS 上做，理由很实际。服务器上只有 Linux 这一种方言，笔记本和它说同一种话，学一遍到处通用。绝大多数生信工具只在 Unix 上开发和测试，安装是一条命令的事，到了 Windows 原生环境常常压根装不上。还有一层历史原因：Unix 当年就是按"小工具加管道"的思路设计出来的，下一节讲的命令行效率，整套思路都是这个家族的发明。
...
实验室的计算分布在三个层次上。笔记本是工作台，写代码、看结果、跑小任务，一切都在本地。服务器是干重活的，内存和硬盘比笔记本大几个数量级，常常还插着 GPU，测序数据直接存在上面，大到根本下载不进笔记本。集群是一群服务器的集合，几十上百台机器由一个调度器统一派活，你把作业用 `sbatch` 交进队列，排到你了才开始跑。

这三层机器的入口是同一个：用 SSH 登进去，面对的都是 shell；文件在它们之间挪动，靠 `scp`、`rsync` 这类命令行工具。服务器上装软件也不用自己动手配环境，实验室用 pixi 集中管理，见[常规实验室支持](../5-lab-code/5.1-routine-support.md)。这也是这一页把命令行放在最前面讲的原因：它是所有这些机器共同的语言，学会一遍，从笔记本到集群处处通用。
...
- [《计算机科学速成课》第 18 集：操作系统](https://www.bilibili.com/video/BV1iK411g7iK/)（B 站精校中字）：十三分钟讲完操作系统为什么被发明出来，从批处理、多任务一路到 Unix 的来龙去脉；同系列第 22 集讲的正好是命令行界面。
- Warren Toomey 的 [The Strange Birth and Long Life of Unix](https://spectrum.ieee.org/the-strange-birth-and-long-life-of-unix)（英文）：IEEE Spectrum 的长文，讲 Unix 怎么从贝尔实验室的一个副业项目长成整个行业今天的地基，故事性很强。
